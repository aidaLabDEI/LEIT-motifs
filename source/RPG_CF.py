...
def conf_sampling(subsequences, i, num_conf, collisions, k):
    ...
    if top.empty():
        ...
        return True, top, dist_comp


def order_hash(hash_mat, l_ret, dimension):
...
def pmotif_findauto(
    ...
    with Pool(processes=int(cpu_count())) as pool:
        ...
        for rep in range(L):
            result = pool.apply_async(
                order_hash, (hash_mat[:, rep, :, :], rep, dimension)
            )
    ...
    for j in range(1, trees_per_forest):
        ...
        for i in range(K - 1):
                ...
                for tree in range(j):
                    ...
                    for dim in range(dimension):
                        ...
                        for idx, elem1 in enumerate(search):
                            ...
                            if colls > max_allowed_collisions:
                                break
            ...
            if forests_ok >= num_forests // 4:
                fin, result, dist = conf_sampling(
                    windowed_ts, i, num_forests // 4, colls_dict, k
                )
                if fin:
                    return result, dist
                else:
