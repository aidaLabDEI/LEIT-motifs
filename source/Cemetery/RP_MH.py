...
def minhash_cycle(i, j, subsequences, hash_mat, k, lsh_threshold):
    ...
    with lsh.insertion_session() as session:
        for ik, signature in enumerate(
            MinHash.generator(pj_ts, num_perm=dimensionality * 2, seed=minhash_seed)
        ):
    ...
    for j_index, minhash_sig in enumerate(minhash_signatures):
        ...
        if len(collisions) >= 1:
            ...
            collisions = [
                sorted((j_index, c))
                for c in collisions
                if c != j_index and abs(c - j_index) > window
            ]
            curr_dist = 0
            ...
            for collision in collisions:
                ...
                if not i == 0:
                    if i == 1:
                        ...
                        rows = (
                            hash_mat[coll_0, j, :, : -i + 1]
                            == hash_mat[coll_1, j, :, : -i + 1]
                        )
                ...
                for stored in top.queue:
                    ...
                    if (
                        abs(coll_0 - stor_0) < window
                        or abs(coll_1 - stor_1) < window
                        or abs(coll_0 - stor_1) < window
                        or abs(coll_1 - stor_0) < window
                    ):
                        # Distance is computed only on distances that match
                        dim = pj_ts[coll_0] == pj_ts[coll_1]
                        dim = np.all(dim, axis=1)
                        dim = [i for i, elem in enumerate(dim) if elem]

                        if len(dim) < dimensionality:
                            break

                        dist_comp += len(dim)
                        curr_dist, dim, stop_dist = z_normalized_euclidean_distance(
                            subsequences.sub(coll_0),
                            subsequences.sub(coll_1),
                            np.array(dim),
                            subsequences.mean(coll_0),
                            subsequences.std(coll_0),
                            subsequences.mean(coll_1),
                            subsequences.std(coll_1),
                            dimensionality,
                        )
                        if curr_dist < stored_dist:
                            top.queue.remove(stored)
                            top.put(
                                (-curr_dist, [dist_comp, collision, [dim], stop_dist])
                            )
                ...
                if add:
                    ...
                    dist_comp += len(dim)
                    distance, dim, stop_dist = z_normalized_euclidean_distance(
                        subsequences.sub(coll_0),
                        subsequences.sub(coll_1),
                        np.array(dim),
                        subsequences.mean(coll_0),
...
def pmotif_find2(
    ...
    shm_hash_mat, hash_mat = create_shared_array(
        (n - window + 1, L, dimension, K), dtype=np.int8
    )
    ...
    with Pool(processes=int(multiprocessing.cpu_count())) as pool:
        ...
        for chunk in chunks:
            result = pool.apply_async(
                process_chunk,
                (*chunk, shm_hash_mat.name, hash_mat.shape, L, dimension, K),
            )
    ...
    def worker(i, j, K, L, r, motif_dimensionality, dimensions, k):
        ...
        if stopped_event.is_set():
            return
        top_i, dist_comp_i = minhash_cycle(
            i, j, windowed_ts, hash_mat, k, lsh_threshold
        )
        element = None
        length = 0
        with lock:
            ...
            for id, elem in enumerate(top.queue):
                for elem2 in top.queue[id + 1 :]:
                    ...
                    if (
                        abs(indices_1[0] - indices_2[0]) < window
                        or abs(indices_1[1] - indices_2[1]) < window
                        or abs(indices_1[0] - indices_2[1]) < window
                        or abs(indices_1[1] - indices_2[0]) < window
                    ):
        ...
        if length == 0:
            ...
            ss_val = stop(
                element,
                motif_dimensionality / dimensions,
                b,
                s,
                i,
                j,
    ...
    with ThreadPoolExecutor(max_workers=int(multiprocessing.cpu_count())) as executor:
        futures = [
            executor.submit(
                worker, i, j, K, L, bin_width, motif_dimensionality, dimension, k
            )
            for i in range(K)
            for j in range(L)
        ]
