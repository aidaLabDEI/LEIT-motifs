...
def RP(
    ...
    chunks = [
        (ts, ranges, transformers, window, c_dimensionality)
        for ranges in np.array_split(ts, num_chunks)
    ]
    ...
    collision_matrix = np.zeros(
        (n - window + 1, n - window + 1)
    )  # dok_array(((n-window+1, n-window+1)), dtype=np.int16) #np.zeros((n-window+1, n-window+1))
    dimensions_sets = list(
        combinations(np.arange(dimensionality), motif_dimensionality)
    )
    random.shuffle(dimensions_sets)

    # Fill the collision matrix
    collision_matrix = fill_matrix(
        collision_matrix, dimensions_sets, sax_results, n, window
    )
    ...
    print("Max elements found:", max_elements.shape[0])
    for maximal in max_elements:
        maximal_sub1, maximal_sub2 = maximal
        dist_comp += 1
        curr_dist, dim, _ = z_normalized_euclidean_distanceg(
