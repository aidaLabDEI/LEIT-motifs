...
if __name__ == "__main__":
    ...
    for i, ds_val in enumerate(ds_values):
        ...
        axs[i//2, i%2].stackplot(L_data['L'], L_data['Time int'], color="peachpuff", alpha=0.8, labels=["Hash time"])
        axs[i//2, i%2].stackplot(L_data['L'], L_data['Time elapsed'], color="wheat", alpha=0.4, labels=["Search time"])
    ...
    for i, ds_val in enumerate(ds_values):
        ...
        axs[i//2, i%2].vlines(r[i], 0, r_dc[i], linestyle='dotted', color='crimson', label= 'tuned r')
        axs[i//2, i%2].set_title(names[i])
