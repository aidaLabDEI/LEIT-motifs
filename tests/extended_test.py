...
def main():
    ...
    for number, path in enumerate(paths):
        ...
        for i in range(3):
            motifs, num_dist = pmotif_findg(shm_ts.name, n, dimensions, windows[number_r], 1, dimensionality[number_r], r_vals_computed[number_r], 0.5, 200, 8)
        ...
        for r in rs:
            start = time.process_time()
            for i in range(1):
                motifs, num_dist = pmotif_findg(shm_ts.name, n, dimensions, windows[number_r], 1, dimensionality[number_r], r, 0.5, 200, 8)
            end = (time.process_time() - start)
