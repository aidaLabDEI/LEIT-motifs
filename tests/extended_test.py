...
def main():
    ...
    for number, path in enumerate(paths):
        number_r = number +5
        results = pd.DataFrame(columns=['Dataset', 'Time elapsed', 'RC1', 'K', 'L', 'w', 'r', 'dist_computed', 'Time int'])
    ...
    for number, path in enumerate(paths):
        ...
        tracemalloc.start()
        _, _,_ = pmotif_findg(shm_ts.name, n, dimensions, windows[number_r], 1, dimensionality[number_r], r_vals_computed[number_r], 0.5, 200, 8)
